...
router.post('/tokens/batch', authMiddleware, async (req, res) => {
  const tokens = req.body;
  if (!Array.isArray(tokens)) {
    return res.status(400).json({ success: false, message: '请求体必须是Token数组' });
  }

  try {
    const result = await tokenManager.batchAddTokens(tokens);
    res.json(result);
  } catch (error) {
    logger.error('批量添加Token失败:', error.message);
    res.status(500).json({ success: false, message: error.message });
  }
});
